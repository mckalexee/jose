import { JOSEAlgNotAllowed, JOSENotSupported, JWEInvalid } from '../../lib/errors.js'
 ...
 * @param key Key, or a function resolving a key, to decrypt the JWE with.
...
export default async function flattenedDecrypt(
  ...
  if ('protected' in jwe && typeof jwe.protected !== 'string') {
    throw new JWEInvalid('JWE Protected Header missing or incorrect type')
  }
  ...
  if (joseHeader.zip === 'DEF') {
    let inflateFn = inflate
    if (options && options.inflateRaw) {
      inflateFn = options.inflateRaw
    }
    plaintext = await inflateFn(plaintext)
  }
