...
import type { FlattenedEncryptClass, JWEKeyManagementHeaderResults } from '../../types.i.d'
...
import { JOSENotSupported, JWEInvalid } from '../../lib/errors.js'
 ...
 * ```
 */
export default class FlattenedEncrypt implements FlattenedEncryptClass<FlattenedJWE> {
  private _plaintext: Uint8Array | Readable
   ...
   */
  constructor(plaintext: Uint8Array | Readable) {
  ...
  async encrypt(key: KeyLike, options?: EncryptOptions) {
    ...
    if (joseHeader.zip === 'DEF') {
      let deflateFn = deflate
      if (options && options.deflateRaw) {
        deflateFn = options.deflateRaw
      }
      const deflated = await deflateFn(this._plaintext)
