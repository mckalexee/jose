...
class RemoteJWKSet {
  ...
  async getKey(protectedHeader: JWSHeaderParameters): Promise<KeyObject | CryptoKey> {
    ...
    const candidates = this._jwks!.keys.filter((jwk) => {
      // filter keys based on the Algorithm to Key Type mapping
      let candidate = jwk.kty === getKtyFromAlg(protectedHeader.alg!)

      // filter keys based on the JWK Key ID
      if (candidate && typeof protectedHeader.kid === 'string' && typeof jwk.kid === 'string') {
  ...
  async reload() {
    if (!this._pendingFetch) {
      this._pendingFetch = fetchJson(this._url, this._timeoutDuration).then(
        (json: { keys: object[] }) => {
          if (
            typeof json !== 'object' ||
            !json ||
            ...
            json.keys.some((key: object) => typeof key !== 'object')
          ) {
